void ball_loop() {
  ...
  for (int i = 0; i<ball_numbers.length; i++) { // For each ball check for changes required
    ...
    // Two state b
    if (effect_balls[i].equals("two_state_b") == true) {
      if (flag_balls[i] == 1) {
        ...
        two_state_b(colours_array,ball_numbers[i]);
        flag_balls[i] = 0;
      }
    }
    ...
    if (effect_balls[i].equals("random_fade") == true) {
      ...
      if ((millis()>timer_balls[i])) {
        ...
        // RANDOM FADE BALL
    if (effect_balls[i].equals("random_fade_b") == true) {
     
      if (flag_balls[i] == 1) {
        random_fade_b(sdrDelay[i].getValueI(), ball_numbers[i]);
        flag_balls[i] = 0;
      }
    }
    
            // RANDOM FADE BALL
    if (effect_balls[i].equals("throw_random_b") == true) {
     
      if (flag_balls[i] == 1) {
        throw_random_b( ball_numbers[i]);
        flag_balls[i] = 0;
      }
    }
